...
type CommandParams = {
  ...
  create_table: {
    ...
    numColumns?: number;
    name?: string;
    cellData?: Array<{ row: number; col: number; text: string }>;
  };
  create_code_block: {
    x?: number;
    y?: number;
...
// Create Sticky Tool (FigJam only)
server.tool(
  "create_sticky",
  "Create a sticky note in FigJam",
  {
    ...
    fillColor: z
      ...
      .optional()
      .describe("Background color in RGBA format"),
  ...
  async ({ x, y, text, fillColor, authorVisible, name }: any) => {
    try {
      const result = await sendCommandToFigma("create_sticky", {
        x, y, text, fillColor, authorVisible, name,
      });
      const typedResult = result as { id: string; name: string; text: string };
      return {
        content: [{
          type: "text",
...
server.tool(
  ...
  "Create a section container in Figma or FigJam",
  {
    ...
    childrenIds: z
      .array(z.string())
      .optional()
      .describe("Array of node IDs to move into the section"),
  },
  async ({ x, y, width, height, name, fillColor, childrenIds }: any) => {
    try {
      const result = await sendCommandToFigma("create_section", {
        x, y, width, height, name, fillColor, childrenIds,
      });
...
// Create Shape With Text Tool (FigJam only)
server.tool(
  "create_shape_with_text",
  "Create a shape with text in FigJam (flowchart nodes, diagrams, etc.)",
  {
    ...
    shapeType: z
      .enum([
        ...
        "PENTAGON", "OCTAGON", "PLUS", "ARROW_LEFT", "ARROW_RIGHT",
        "SUMMING_JUNCTION", "SPEECH_BUBBLE", "INTERNAL_STORAGE", "STAR", "OR",
      ])
      .optional()
      .describe("Shape type (default ROUNDED_RECTANGLE)"),
    ...
    fillColor: z
      ...
      .optional()
      .describe("Shape background color in RGBA format"),
    name: z.string().optional().describe("Layer name"),
  },
  async ({ x, y, shapeType, text, fillColor, name }: any) => {
    try {
      const result = await sendCommandToFigma("create_shape_with_text", {
        x, y, shapeType, text, fillColor, name,
      });
      const typedResult = result as { id: string; name: string; shapeType: string };
      return {
        content: [{
          type: "text",
...
server.tool(
  ...
  "Create a table in FigJam",
  {
    ...
    cellData: z
      ...
      .optional()
      .describe("Array of cell data to populate the table"),
  },
  async ({ x, y, numRows, numColumns, name, cellData }: any) => {
    try {
      const result = await sendCommandToFigma("create_table", {
        x, y, numRows, numColumns, name, cellData,
      });
      const typedResult = result as { id: string; name: string; numRows: number; numColumns: number };
      return {
        content: [{
          type: "text",
...
// Create Code Block Tool (FigJam only)
server.tool(
  "create_code_block",
  "Create a code block in FigJam with syntax highlighting",
  {
    ...
    codeLanguage: z
      .enum([
        ...
        "GRAPHQL", "PYTHON", "GO", "SQL", "SWIFT", "KOTLIN", "RUST",
        "BASH", "PLAINTEXT", "DART",
      ])
      .optional()
      .describe("Programming language for syntax highlighting (default PLAINTEXT)"),
    name: z.string().optional().describe("Layer name"),
  },
  async ({ x, y, code, codeLanguage, name }: any) => {
    try {
      const result = await sendCommandToFigma("create_code_block", {
        x, y, code, codeLanguage, name,
      });
      const typedResult = result as { id: string; name: string; codeLanguage: string };
      return {
        content: [{
          type: "text",
...
// Create Link Preview Tool (FigJam only)
server.tool(
  "create_link_preview",
...
server.tool(
  ...
  async ({ nodeId, url, nodeIdTarget, start, end }: any) => {
    try {
      const result = await sendCommandToFigma("set_hyperlink", {
        nodeId, url, nodeIdTarget, start, end,
      });
      const typedResult = result as { id: string; name: string; hyperlink: any };
      const linkDesc = typedResult.hyperlink
        ? (typedResult.hyperlink.type === "URL" ? typedResult.hyperlink.value : `node:${typedResult.hyperlink.value}`)
        : "removed";
      return {
        content: [{
          type: "text",
...
server.tool(
  ...
  async ({ url, imageData, x, y, width, height, scaleMode, name }: any) => {
    try {
      const result = await sendCommandToFigma("create_image", {
        url, imageData, x, y, width, height, scaleMode, name,
      });
